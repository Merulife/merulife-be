...
/**
 * Handles sending a message (or transcript) for a specific companion.
 ...
 * Accepts an additional field `type` (e.g., "CHAT" or "VOICE"), defaulting to "CHAT".
 */
export const sendMessage = async (req: Request, res: Response) => {
  try {
    const { companionId } = req.params;
    // Expecting message, sender, userId, and optional type in the body.
    const { message, sender, userId, type } = req.body;
    ...
    // Create the chat message, using the provided type or defaulting to "CHAT".
    const chatMessage = await prisma.chatMessage.create({
      data: {
        conversationId: conversation.id,
        content: message,
        sender,
        type: type || "CHAT", // Accept "CHAT" or "VOICE"
      },
    });
...
/**
 * Retrieves the last ten conversations.
 */
